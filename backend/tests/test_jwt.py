...
def test_mint_and_decode_roundtrip() -> None:
    ...
    token = mint_jwt(user_id=user_id, is_admin=False, email_verified=True)
...
def test_bad_signature_returns_none() -> None:
    ...
    token = mint_jwt(user_id="user-x", is_admin=True, email_verified=True)
...
def test_token_payload_fields() -> None:
    ...
    token = mint_jwt(user_id="u", is_admin=False, email_verified=False)
